...
class PayApiController extends Controller
{
    ...
    public function agentPay(Request $request)
    {
        ...
        $data['sign']= MD5('amount='.$data['amount'].'&bankCardholder='.$data['bankCardholder'].'&bankCardNo='.$data['bankCardNo'].'&bankCode='.$data['bankCode'].'&city='.$data['city'].'&extra='.$data['extra'].'&notifyUrl='.$data['notifyUrl'].'&partner='.$data['partner'].'&province='.$data['province'].'&service='.$data['service'].'&subbranch='.$data['subbranch'].'&subsidiaryBank='.$data['subsidiaryBank'].'&tradeNo='.$data['tradeNo'].'&'.$this->key);
